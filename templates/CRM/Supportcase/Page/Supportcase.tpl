...
{if !$isCaseLocked}
  ...
  {else}
    ...
    {elseif $action eq 4 }
      {include file="CRM/Supportcase/Form/Supportcase.tpl"}
  ...
  <script>
      CRM.$(function ($) {
          ...
          var mangeCaseUpdateLockTime = '{/literal}{$mangeCaseUpdateLockTime}{literal}' * 1000;
          initManageLocker();

          $('.ui-button.ui-dialog-titlebar-close').click(function(){
              clearInterval(window.supportCaseTimer);
              console.log('Locking timer clear');
          });

          function initManageLocker() {
              console.log(window.supportCaseTimer);
              clearInterval(window.supportCaseTimer);
              window.supportCaseTimer = setInterval(updateManageCaseLocking, mangeCaseUpdateLockTime);
          }

          function updateManageCaseLocking() {
              console.log(caseId);

              CRM.api3('CaseLock', 'lock_case', {
                  "case_id": caseId
              }).then(function(result) {
                  console.log('case (id = ' + caseId + ') is locked!!!');
              }, function(error) {});
          }
